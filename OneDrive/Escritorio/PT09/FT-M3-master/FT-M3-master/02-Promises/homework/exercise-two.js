...
function problemC () {
  ...
  // promise version
  // ???
  filenames.reduce(( p, file) => { //p es el acumulador, y file son los archivos a acumular cada stanza
   return p.then((stanza) => {
    if(stanza) blue(stanza);
    return promisifiedReadFile(file)  //p=promisifiedReadfile['poem-two/stanza-01.txt']
   })
